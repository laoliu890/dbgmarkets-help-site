...
if (!existsSync(siteDir)) {
  ...
  if (existsSync(sitemapPath)) {
    ...
    for (const route of coreRoutes) {
      const url = `https://dbgmarketshelp.com${route === "/" ? "/" : route}`;
      if (!sitemapUrls.has(url)) errors.push(`sitemap.xml 缺少：${url}`);
    }
    if (sitemapUrls.has("https://dbgmarketshelp.com/404.html")) {
  ...
  if (existsSync(robotsPath)) {
    ...
    if (!/Sitemap:\s*https:\/\/dbgmarketshelp\.com\/sitemap\.xml/i.test(robots)) {
  ...
  for (const file of htmlFiles) {
    ...
    if (!/<meta\s+property=["']og:image["']\s+content=["']https:\/\/dbgmarketshelp\.com\/[^"']+["']/i.test(html)) {
    ...
    if (!/<meta\s+name=["']twitter:image["']\s+content=["']https:\/\/dbgmarketshelp\.com\/[^"']+["']/i.test(html)) {
